 ...
 * @param {Object} [config.customHandlers] - Custom event handlers mapped by 'channel:event'.
 *
 * @returns {Object} - An object containing the publish function.
 */
const useBaseComponent = ({
     ...
     * Maps events to custom handler functions based on 'channel:event' keys.
     *
    ...
    const handleEvent = (eventObj) => {
        ...
        if (customHandlers[key] && typeof customHandlers[key] === 'function') {
            ...
            console.warn(`No handler found for ${key} in ${name}.`)
        }
    }
